...
# iter <- iter(chunk_size = 100, iter(1:100))
# foreach(i = iter,.combine = "c")%do%
#   {
...
foreach(g = 1:length(gamma))%do%{
  ...
  Data = function()
  {
    DataMatrix1 = generatet(n = n_train,df = df,mu = Mu_train1,S = Sigma_train1,
                            perout = percent,gamma = gamma[g],outlierType = "cellwisePlain")$X
    ...
    DataMatrix2 = rmvt(n = n_train,mu = Mu_train2,S = Sigma_train2,df = df)
    ...
    TDataMatrix2 = rmvt(n = n_test,mu = Mu_test2,S = Sigma_test2,df = df)
